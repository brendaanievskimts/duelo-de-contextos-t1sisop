...
# Conclusão
  ...
  Em relação ao overhead de criação, processos apresentam custo maior do que threads: cada chamada fork() implica a criação de um novo espaço de endereçamento e a cópia das estruturas do processo pai, enquanto threads são criadas dentro do mesmo espaço de memória com custo consideravelmente menor. Nos experimentos realizados, essa diferença não se destacou nos tempos totais medidos, pois o custo de criação foi amortizado pelo tempo de execução do contador em si, mas em aplicações com criação frequente de unidades de execução essa distinção se tornaria muito mais perceptível.
  
  Quanto à eficiência na comunicação, threads levam vantagem natural por compartilharem diretamente a memória do processo - o acesso à variável global é imediato e sem overhead de IPC. Processos, por sua vez, dependem de Memória Compartilhada (shmget/shmat) e Semáforos (sem_open), mecanismos que introduzem chamadas de sistema adicionais. Neste experimento os tempos foram muito próximos entre os dois modelos, sugerindo que o gargalo principal foi a contenção no contador e não o mecanismo de comunicação; ainda assim, threads são o modelo mais eficiente para comunicação em cenários de dados compartilhados.
  
  Conclui-se que threads apresentaram menor overhead de criação e maior eficiência na comunicação, sendo mais indicadas para tarefas que exigem compartilhamento intenso de dados - desde que acompanhadas de sincronização adequada. Processos, embora com maior custo de criação e comunicação via IPC, oferecem maior isolamento e segurança, sendo preferíveis quando a independência entre as unidades de execução é prioritária.
